...
def ecartsol(tf,n,fonction,i):    # tentative de reduction du temps de ecartsol3   
    ...
    for j in range(4): # car on teste pour 4 valeurs de pas, donc pour les listes T1,T2,T3,et T4 respectivement en positions de 0 à 3 dans la liste LT
        ...
        for k in Tref: # pour chaque valeur de Treference
            ...
            if k!=Tref[0] and k!=Tref[-1]:           # si k n'est ni 0 ni 1000, on etend notre recherche aux positions de LT entre positionsusceptible-10 et position susceptible+10
                posdepart=positionsusceptible-10 # ainsi, dans la première for bouclera 20 fois au lieu des len(LT) fois. ce qui est un enorme gain de temps
    ...
    for i in Tableau:
        if i[0]!=vraiTableau[-1][0]:
            vraiTableau.append(i)
    print(vraiTableau) # ce tableau renvoie, pour chaque valeur de Tref, et pour chaque  j (qu'on relie au pas) , la valeur de l'imprecision (pour les Angles)
    ...
    fenetre.title("creation code fenetre")
    ...
    for a in range(len(P)):
        if P[a] in Lj2: 
            ...
            pos=a+1
        Lj2.append(P[a])
        Pas=Label(c,text=P[a])
        Pas.grid(row=pos,column=0)
        Lposrow.append([a,pos])
        
    for a in range(len(vraiTableau)):
        posrow=0
        poscol=0
        for b in Lposrow:
            if vraiTableau[a][2]==b[0]:
                posrow=b[1]
        for d in Lposcol:
            if vraiTableau[a][2]==d[0]:
                poscol=d[1]
        print(vraiTableau[a][2])
        Ecart=Label(c,text=vraiTableau[a][2])
