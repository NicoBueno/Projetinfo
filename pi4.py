...
def compare3(tf,n,i):   #trace les rayons et angles obtenus avec la solution exacte
    T,Y0,A=solexacte(tf,n,0)
    T,Y1,A=solexacte(tf,n,1)
    T,Y2,A=solexacte(tf,n,2)
    T,Y3,A=solexacte(tf,n,3)
    T,Y4,A=solexacte(tf,n,4)
    R=[Rt for i in range(len(T))]
    plt.plot(T,Y0,'r',label="V(t=0)=5491 m/s")
    plt.plot(T,Y1,'g',label="V(t=0)=5491 m/s")
    plt.plot(T,Y2,'b',label="V(t=0)=5491 m/s")
...
def compare5(tf,n,i):  #compare les différentes solutions en fonction de la vitesse initiale et du nombre de points d'acquisitions
    T,Y,A=recurrence(tf,n,i)
    Ts,Ys,As=solscipy(tf,n,i)
    Te,Ye,Ae=solexacte(tf,n,i)
